...
exports.createSale = async (req, res) => {
  ...
  const autoGenerateDocuments = req.body.auto_generate_documents === true;
  ...
  try {
    ...
    // הפעלת תהליך הפקת מסמכים אם נדרש
    if (autoGenerateDocuments) {
      try {
        // ייבוא השירות החדש
        const InvoiceShippingService = require('../services/invoiceShippingService');
        
        // הפעלת התהליך ברקע (ללא המתנה)
        setImmediate(async () => {
          try {
            await InvoiceShippingService.processSaleComplete(saleId);
        ...
        console.error('שגיאה בייבוא שירות הפקת מסמכים:', error);
      }
    }
    ...
    res.status(201).json({
      ...
      message: hasUnsupplied
        ...
        : 'המכירה בוצעה בהצלחה.',
      documents_processing: autoGenerateDocuments
    });
  } catch (err) {
